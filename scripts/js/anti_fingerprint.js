(function() {
    'use strict';

    let _attempts = 0;
    function _reportTrack(method) {
        _attempts++;
        console.log('[JuanitaBanana] Fingerprinting attempt detected via ' + method + ' (Total: ' + _attempts + ')');
        try {
            window.top.postMessage({
                type: 'juanita_track_or_ad',
                isAd: false,
                count: 1
            }, '*');
        } catch(e) {}
    }

    function _reportAndExecute(method, fn) {
        _reportTrack(method);
        return fn();
    }

    // ── Prototype toString Protection ─────────────────────
    // Overrides Function.prototype.toString to return standard native function representation
    // ("function () { [native code] }" or "function name() { [native code] }") for all spoofed APIs.
    const _origToString = Function.prototype.toString;
    const _nativeFnMap = new WeakMap();

    function _makeNative(fn, name) {
        if (typeof fn === 'function') {
            const fnName = name || fn.name || '';
            if (name && fn.name !== name) {
                try {
                    Object.defineProperty(fn, 'name', {
                        value: name,
                        writable: false,
                        enumerable: false,
                        configurable: true
                    });
                } catch(e) {}
            }
            const str = fnName ? `function ${fnName}() { [native code] }` : 'function () { [native code] }';
            _nativeFnMap.set(fn, str);
        }
        return fn;
    }

    const _protectedToString = function toString() {
        if (_nativeFnMap.has(this)) {
            _reportTrack('Function.prototype.toString (' + (this.name || 'anonymous') + ')');
            return _nativeFnMap.get(this);
        }
        return _origToString.call(this);
    };

    _makeNative(_protectedToString, 'toString');
    try {
        Object.defineProperty(Function.prototype, 'toString', {
            value: _protectedToString,
            writable: true,
            enumerable: false,
            configurable: true
        });
    } catch(e) {
        Function.prototype.toString = _protectedToString;
    }

    function _defineGetter(obj, prop, getterFn) {
        const nativeGetter = _makeNative(getterFn, 'get ' + prop);
        Object.defineProperty(obj, prop, {
            get: nativeGetter,
            configurable: true,
            enumerable: true
        });
        return nativeGetter;
    }

    function _overrideMethod(target, prop, newFn) {
        const nativeFn = _makeNative(newFn, prop);
        target[prop] = nativeFn;
        return nativeFn;
    }

    // ── Viewport Fingerprinting ──────────────────────────
    const _randInt = (n) => Math.floor(Math.random() * n);
    const fakeWidth  = screen.width  + _randInt(50) - 25;
    const fakeHeight = screen.height + _randInt(50) - 25;

    _defineGetter(screen, 'width',       () => _reportAndExecute('screen.width', () => fakeWidth));
    _defineGetter(screen, 'height',      () => _reportAndExecute('screen.height', () => fakeHeight));
    _defineGetter(screen, 'availWidth',  () => _reportAndExecute('screen.availWidth', () => fakeWidth));
    _defineGetter(screen, 'availHeight', () => _reportAndExecute('screen.availHeight', () => fakeHeight));
    _defineGetter(window, 'innerWidth',  () => _reportAndExecute('window.innerWidth', () => fakeWidth));
    _defineGetter(window, 'innerHeight', () => _reportAndExecute('window.innerHeight', () => fakeHeight));

    // ── Canvas Fingerprinting ────────────────────────────
    const _fakeDataURL = 'data:image/jpeg;base64,';
    _overrideMethod(HTMLCanvasElement.prototype, 'toDataURL', function toDataURL() { return _reportAndExecute('canvas.toDataURL', () => _fakeDataURL); });
    _overrideMethod(HTMLCanvasElement.prototype, 'toBlob', function toBlob(cb) { return _reportAndExecute('canvas.toBlob', () => { fetch(_fakeDataURL).then(r=>r.blob()).then(cb); }); });
    const _origGetImageData = CanvasRenderingContext2D.prototype.getImageData;
    _overrideMethod(CanvasRenderingContext2D.prototype, 'getImageData', function getImageData(x, y, w, h) {
        return _reportAndExecute('canvas.getImageData', () => {
            const d = _origGetImageData.call(this, x, y, w, h);
            for (let i = 0; i < d.data.length; i += 4) { d.data[i] ^= _randInt(3); }
            return d;
        });
    });

    // ── WebGL Fingerprinting ─────────────────────────────
    const _origGetParam = WebGLRenderingContext.prototype.getParameter;
    _overrideMethod(WebGLRenderingContext.prototype, 'getParameter', function getParameter(param) {
        if (param === 37445) return _reportAndExecute('webgl.vendor', () => 'Juanita Banana GPU');            // UNMASKED_VENDOR_WEBGL
        if (param === 37446) return _reportAndExecute('webgl.renderer', () => 'Juanita Banana Graphics API');   // UNMASKED_RENDERER_WEBGL
        return _origGetParam.call(this, param);
    });

    // ── Navigator Fingerprinting ─────────────────────────
    _defineGetter(navigator, 'hardwareConcurrency', () => _reportAndExecute('navigator.hardwareConcurrency', () => 4 + _randInt(4)));
    _defineGetter(navigator, 'deviceMemory',        () => _reportAndExecute('navigator.deviceMemory', () => 8));
    _defineGetter(navigator, 'platform',            () => _reportAndExecute('navigator.platform', () => 'Linux x86_64'));
    _defineGetter(navigator, 'vendor',              () => _reportAndExecute('navigator.vendor', () => 'Juanita Banana'));
    _defineGetter(navigator, 'userAgent',           () => _reportAndExecute('navigator.userAgent', () => 'JuanitaBanana/0.1 (FOSS; Not-Google; Linux)'));

    // Bot detection bypass: these fields expose automation
    _defineGetter(navigator, 'webdriver', () => _reportAndExecute('navigator.webdriver', () => false));
    _defineGetter(navigator, 'languages', () => _reportAndExecute('navigator.languages', () => ['en-US', 'en']));
    _defineGetter(navigator, 'plugins',   () => _reportAndExecute('navigator.plugins', () => Object.setPrototypeOf([
        { name: 'PDF Viewer', filename: 'internal-pdf-viewer', description: 'Portable Document Format' },
        { name: 'Chrome PDF Viewer', filename: 'mhjfbmdgcfjbbpaeojofohoefgiehjai', description: '' },
    ], PluginArray.prototype)));

    // ── Intl / Geolocation Leak ──────────────────────────
    const _origDateTimeFormat = Intl.DateTimeFormat;
    const _spoofedDateTimeFormat = function DateTimeFormat(locales, options) {
        if (options) { options.timeZone = 'Europe/London'; }
        else { options = { timeZone: 'Europe/London' }; }
        return new _origDateTimeFormat(locales, options);
    };
    _makeNative(_spoofedDateTimeFormat, 'DateTimeFormat');
    _spoofedDateTimeFormat.prototype = _origDateTimeFormat.prototype;
    _spoofedDateTimeFormat.supportedLocalesOf = _makeNative(function supportedLocalesOf(locales, options) {
        return _origDateTimeFormat.supportedLocalesOf(locales, options);
    }, 'supportedLocalesOf');
    Intl.DateTimeFormat = _spoofedDateTimeFormat;

    // ── Battery API Spoofing ─────────────────────────────
    _overrideMethod(navigator, 'getBattery', function getBattery() {
        return _reportAndExecute('navigator.getBattery', () => Promise.resolve({
            charging: true,
            chargingTime: 0,
            dischargingTime: Infinity,
            level: 1.0,
            addEventListener: _makeNative(function addEventListener() {}, 'addEventListener'),
            removeEventListener: _makeNative(function removeEventListener() {}, 'removeEventListener'),
            dispatchEvent: _makeNative(function dispatchEvent() { return true; }, 'dispatchEvent'),
            onchargingchange: null,
            onchargingtimechange: null,
            ondischargingtimechange: null,
            onlevelchange: null
        }));
    });

    // ── Font Enumeration Protection ──────────────────────
    const _origMeasureText = CanvasRenderingContext2D.prototype.measureText;
    _overrideMethod(CanvasRenderingContext2D.prototype, 'measureText', function measureText(text) {
        return _reportAndExecute('canvas.measureText', () => {
            const originalFont = this.font;
            let forcedFont = 'monospace';
            if (originalFont.toLowerCase().includes('webdings')) {
                forcedFont = 'webdings';
            }
            let size = '10px';
            const sizeMatch = originalFont.match(/(\d+(?:\.\d+)?(?:px|em|pt|rem))/i);
            if (sizeMatch) { size = sizeMatch[1]; }
            this.font = size + ' ' + forcedFont;
            const metrics = _origMeasureText.call(this, text);
            this.font = originalFont;
            return metrics;
        });
    });

    if (window.FontFaceSet && FontFaceSet.prototype.check) {
        _overrideMethod(FontFaceSet.prototype, 'check', function check(font) {
            return _reportAndExecute('FontFaceSet.check', () => {
                const lower = font.toLowerCase();
                return lower.includes('webdings') || lower.includes('monospace') || lower.includes('mono') || lower.includes('sans-serif') || lower.includes('serif');
            });
        });
    }

    // ── Sensor API Poisoning ─────────────────────────────
    if (window.DeviceMotionEvent) {
        _reportTrack('DeviceMotionEvent.prototype overrides');
        Object.defineProperties(DeviceMotionEvent.prototype, {
            acceleration: {
                get: _makeNative(function() {
                    const now = Date.now();
                    const x = Math.sin(now / 1000) * 1.5;
                    const y = Math.cos(now / 1000) * 1.5;
                    return { x, y, z: 0 };
                }, 'get acceleration')
            },
            accelerationIncludingGravity: {
                get: _makeNative(function() {
                    const now = Date.now();
                    const x = Math.sin(now / 1000) * 1.5;
                    const y = Math.cos(now / 1000) * 1.5;
                    const z = 9.8 + (Math.random() - 0.5) * 0.2;
                    return { x, y, z };
                }, 'get accelerationIncludingGravity')
            },
            rotationRate: {
                get: _makeNative(function() {
                    const now = Date.now();
                    return {
                        alpha: Math.sin(now / 500) * 5,
                        beta: Math.cos(now / 500) * 5,
                        gamma: 0
                    };
                }, 'get rotationRate')
            }
        });
    }

    if (window.DeviceOrientationEvent) {
        _reportTrack('DeviceOrientationEvent.prototype overrides');
        Object.defineProperties(DeviceOrientationEvent.prototype, {
            alpha: { get: _makeNative(function() { return (Date.now() / 50) % 360; }, 'get alpha') },
            beta: { get: _makeNative(function() { return 45 + Math.sin(Date.now() / 2000) * 10; }, 'get beta') },
            gamma: { get: _makeNative(function() { return Math.cos(Date.now() / 2000) * 10; }, 'get gamma') },
            absolute: { get: _makeNative(function() { return true; }, 'get absolute') }
        });
    }

    if (window.Sensor || !window.Accelerometer) {
        const createSensorMock = (className, getValueFn) => {
            class MockSensor extends EventTarget {
                constructor(options) {
                    super();
                    this.activated = false;
                    this.hasReading = false;
                    this.frequency = (options && options.frequency) || 10;
                    this.onreading = null;
                    this.onactivate = null;
                    this.onerror = null;
                }
                start() {
                    _reportTrack(className + '.start');
                    if (this.activated) return;
                    this.activated = true;
                    setTimeout(() => {
                        this.dispatchEvent(new Event('activate'));
                        if (this.onactivate) this.onactivate();
                        this._interval = setInterval(() => {
                            const values = getValueFn();
                            Object.assign(this, values);
                            this.hasReading = true;
                            this.dispatchEvent(new Event('reading'));
                            if (this.onreading) this.onreading();
                        }, 1000 / this.frequency);
                    }, 10);
                }
                stop() {
                    _reportTrack(className + '.stop');
                    this.activated = false;
                    clearInterval(this._interval);
                }
            }
            _makeNative(MockSensor.prototype.start, 'start');
            _makeNative(MockSensor.prototype.stop, 'stop');
            return _makeNative(MockSensor, className);
        };

        const getAccelValues = () => {
            const now = Date.now();
            return {
                x: Math.sin(now / 1000) * 1.5,
                y: Math.cos(now / 1000) * 1.5,
                z: 9.8 + (Math.random() - 0.5) * 0.2
            };
        };

        const getGyroValues = () => {
            const now = Date.now();
            return {
                x: Math.sin(now / 500) * 5,
                y: Math.cos(now / 500) * 5,
                z: 0
            };
        };

        window.Accelerometer = createSensorMock('Accelerometer', getAccelValues);
        window.LinearAccelerationSensor = createSensorMock('LinearAccelerationSensor', getAccelValues);
        window.GravitySensor = createSensorMock('GravitySensor', () => ({ x: 0, y: 0, z: 9.8 }));
        window.Gyroscope = createSensorMock('Gyroscope', getGyroValues);
    }

    console.log('[JuanitaBanana] Anti-fingerprint active 🍌');
})();
