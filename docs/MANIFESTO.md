...
Some trackers enumerate installed fonts to create a fingerprint. **Strategy:** Override canvas text measurements and CSS font loading APIs to report that the system *only* has two fonts installed: `Webdings` and `Monospace` (until we implement the session-randomized font pool).
...
- **CAdES Digital Signing** — When a user provides an eIDAS-qualified PKCS#12 certificate (FNMT, idCAT, Izenpe, D-Trust, Aruba PEC, etc.), formal complaints are wrapped in a CAdES `.p7m` signed envelope (ETSI TS 101 903 / RFC 5652). The private key is parsed and used entirely in RAM via the `openssl` Rust crate — it is **never written to disk** at any point, protecting against SSD wear-leveling data retention. The resulting signed document is legally equivalent to a handwritten signature across all EU member states under eIDAS Regulation 910/2014 Art. 3(12).
- **DPA Auto-Submission (Roadmap)** — Future versions will auto-submit the signed complaint directly to the relevant national DPA's online portal (AEPD, Garante, CNIL, BfDI, ICO, DPC, APD/GBA, EDPB), scraping and auto-filling their HTML submission forms without requiring manual copy-paste.
