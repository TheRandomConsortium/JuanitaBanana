...
| **TOTP / Time-Based 2FA** | 🔭 Future | Store TOTP secret keys in the encrypted vault and derive one-time codes on demand. **Intentionally deferred:** storing 2FA on the same device as the account credential undermines the second-factor guarantee. Will only be implemented after a clear threat-model rationale (e.g. dedicated hardware token emulation mode). |
